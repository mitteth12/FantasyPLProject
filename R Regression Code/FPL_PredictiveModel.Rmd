...
Mo Salah had the highest amount of total points for both our model and for the actual totals. However, Heung-Min Son really gained on Salah which means that if we split the season up into halves Salah would have definitely been the highest point contributor in the league and the second half would have been all Heung-Min Son. Rounding out the top 5 was Kane, De Bruyne and Alexander-Arnold. Tottenham made a heavy push in the second half and it can be seen that from the total amount of points that at least Son and Kane made a big impact for Tottenham. 

Looking specifically by position, starting with defenders we have a very interesting top 5 list. We have Joao Cancelo and then we have Liverpool's whole back line which speaks volumes to their contribution to their team this season. A lot of top teams are represented in the defender's list, but a couple of the lower table teams that had some stand out players were Matty Cash and Pontius Jansson for Aston Villa and Brentford respectively. As expected none of the relegated clubs have any defenders grazing the top spots.

Now looking at midfielders we continue to see a similar trend with top teams being represented. One standout player in this group was James Ward-Prowse whose team, Southampton, finished 15th in the league while he managed to finish sixth highest for midfielders. Not a whole lot of overlapping of clubs it looks like for the most part each club has one or two midfielders who really got things done for their teams besides Manchester City. Manchester City, the league winners had five midfielders who were in the top 20 for predicted points this season. This is an example illustrating the depth of Manchester City's roster. Influence and threat look to still be a huge component of picking total points for players. An interesting column that was developed was total goal contributions and that seemed to lineup fairly well with the players who were at the top of the list. This makes it relatively easy for picking midfielders in that you should pick the players getting the most goals and assists.

Lastly we will look at forwards. Forwards turned out to be an interesting one as two of the three relegated sides are represented in the top 10 for forwards. This alone does not necessarily indicate much. It could be that without these players they would have done way worse, but it could also just speak volumes to the fact that if the clubs defense were a little better it could have had more influence on the team staying in the league. We see Kane and Ronaldo holding the top spots for forwards. We have such a higher volume of midfielders and defenders than we do with forwards, so it would make sense that we only see about one player being represented for each team.

Overall, there are a lot more smaller details that could be further evaluated in the future with week by week tracking. We will end this markdown by applying our model to our final data to see how our model performed.
...
In evaluating how the variables did in predicting our players we want to look at how the actual rank is explained by the variables. It can be seen that all of our model variables have some significance besides 'bonus'.
